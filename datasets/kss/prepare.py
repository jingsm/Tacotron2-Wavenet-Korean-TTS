...
def read_csv(path,fn_encoding='UTF8'):
    ...
    with open(path, encoding=fn_encoding) as f:
        ...
        for line in f:
            audio_name, audio_transcript,audio_normalized_transcript,_ = line.split('|', maxsplit=3)
            # arr_line = line.split('|')
            # audio_name = arr_line[0]
            # audio_transcript = arr_line[1]
            # audio_normalized_transcript = arr_line[2]
